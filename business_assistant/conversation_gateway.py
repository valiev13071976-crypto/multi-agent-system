...
class WorkflowPandaConversationGateway:
    ...
    def _get_or_create_family_excel_task(self, *, tenant_id: str, owner_id: str, conversation_id: str):
        """Shared helper for the two managed-agent/legacy-engine state-
        ownership-bridge methods below: returns this conversation's
        existing FAMILY_EXCEL ``ActiveTask`` (Panda's ONE canonical owner
        of "current business task + current dataset", already used by the
        legacy ``ROW_FOUND``/``CALL_PRODUCT_ENRICHMENT`` handlers above),
        creating a fresh one with the SAME shape
        ``_persist_managed_agent_product_context`` already builds if none
        exists yet or an unrelated family is active. Never a new state
        store/schema."""
        from business_assistant.action_continuation import (
            EXCEL_CONTRACT,
            FAMILY_EXCEL,
    ...
    async def _maybe_handle_bulk_table_operation(
        ...
        """Business-task-ownership/workset-continuation defect closure,
        PART 2: prevents the production ``MaxTurnsExceeded`` failure mode.
