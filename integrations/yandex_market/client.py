"""Bounded Yandex Market HTTP client — governed endpoints only."""
...
from integrations.yandex_market.config import YandexMarketIntegrationConfig
from integrations.yandex_market.errors import YandexMarketIntegrationError


class YandexMarketHttpClient:
    ALLOWED_HOSTS = frozenset({"api.partner.market.yandex.ru", "partner.market.yandex.ru"})

    def __init__(self, *, config: YandexMarketIntegrationConfig, secret_resolver: Callable[[str], str | None] | None = None):
        self._config = config
        self._secret_resolver = secret_resolver
        self._http = BoundedHttpClient(provider_id="yandex_market", timeout_seconds=config.timeout_seconds)

    def _assert_host(self, url: str) -> None:
        host = (urlparse(url).hostname or "").lower()
        if host not in self.ALLOWED_HOSTS:
            raise YandexMarketIntegrationError("ssrf_host_not_allowed")
    ...
    def get(self, path: str, *, credential_ref: str = "") -> dict:
        if not self._config.is_live:
            raise YandexMarketIntegrationError("INTEGRATION_ENVIRONMENT_MISMATCH")
        url = self._config.api_base_url.rstrip("/") + path
        self._assert_host(url)
        token = self._token(credential_ref)
        if not token:
            raise YandexMarketIntegrationError("INTEGRATION_NOT_CONFIGURED")
        try:
            resp = self._http.request("GET", url, headers={"Authorization": f"OAuth {token}"})
        ...
        try:
            ...
            raise YandexMarketIntegrationError("INTEGRATION_MALFORMED_RESPONSE") from exc
    ...
    def _raise_normalized(self, exc: ProductionProviderError) -> None:
        ...
        raise YandexMarketIntegrationError(str(exc.category.value)) from exc
