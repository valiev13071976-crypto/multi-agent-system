"""LIVE 1C adapter — structurally capable, dormant without configuration."""
...
from integrations.onec.client import OneCHttpClient


class LiveOneCAdapter(OneCFixtureAdapter):
    def __init__(
        self,
        *,
        config: OneCIntegrationConfig | None = None,
        secret_resolver: Callable[[str], str | None] | None = None,
    ):
        super().__init__()
        self._config = config or load_onec_config()
        self._secret_resolver = secret_resolver
        self._client: OneCHttpClient | None = None
        self.environment = "LIVE"
        self.live = True

    @property
    def client(self) -> OneCHttpClient:
        if self._client is None:
            self._client = OneCHttpClient(config=self._config, secret_resolver=self._secret_resolver)
        return self._client

    def _assert_live_configured(self, credential_ref: str = "") -> None:
        if not self._config.is_live:
            raise IntegrationNotConfiguredError("onec_live_mode_required")
        if not self._config.live_configured:
            raise IntegrationNotConfiguredError("onec_endpoint_not_configured")
    ...
    def read(self, *, capability: str, params: dict | None = None, tenant_id: str = "", credential_ref: str = "") -> dict:
        self._assert_live_configured(credential_ref)
        self._raise_if_bad()
        data = self.client.call("/hs/panda/nomenclature", credential_ref=credential_ref, method="GET")
        return {
    ...
    def write(self, *, capability: str, payload: dict, idempotency_key: str, tenant_id: str = "", credential_ref: str = "") -> dict:
        self._assert_live_configured(credential_ref)
        raise IntegrationNotConfiguredError("onec_live_write_blocked_engineering")
