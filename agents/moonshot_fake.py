"""Deterministic FakeMoonshotProvider — offline only, no network."""
...
class FakeMoonshotProvider:
    """Offline stand-in for MoonshotAgent.run."""
    ...
    async def run(self, prompt: str):
        ...
        mode = self.mode
        if mode == "timeout":
            raise MoonshotProviderError(MOONSHOT_TIMEOUT, retryable=True)
        if mode == "429":
            self.retry_count += 1
            raise MoonshotProviderError(MOONSHOT_RATE_LIMIT, retryable=True, status_code=429)
        if mode == "auth":
            raise MoonshotProviderError(MOONSHOT_AUTH, retryable=False, status_code=401)
        if mode == "malformed":
            raise MoonshotProviderError(MOONSHOT_MALFORMED_RESPONSE)
        if mode == "context_too_large":
            raise MoonshotProviderError(MOONSHOT_CONTEXT_LENGTH, status_code=413)
        ...
        if mode == "tool_instruction":
            # Adapter must not invoke tools even if text asks for it.
            return ProviderResult(
