"""SideEffectExecutor bridge for B2B / Telegram external writes (Block 13)."""

from __future__ import annotations

import uuid

from b2b_commerce.capabilities import CAP_B2B_ORDER_SUBMIT, CAP_B2B_QUOTE_SEND, CAP_TELEGRAM_SEND
...
from tools.models import TOOL_TRUST_WRITE_EXTERNAL_IRREVERSIBLE, TOOL_TRUST_WRITE_EXTERNAL_REVERSIBLE, ToolRequest

B2B_RESOURCE_PREFIX = "b2b:"

B2B_WRITE_TOOLS: tuple[dict, ...] = (
    {
...
class B2BCommerceSideEffectAdapter:
    def __init__(
        self,
        *,
        tool_id: str,
        ...
        self._reversible = reversible if reversible is not None else (trust_level != TOOL_TRUST_WRITE_EXTERNAL_IRREVERSIBLE)
        self._descriptor = SideEffectToolDescriptor(
            ...
            resource_prefix=B2B_RESOURCE_PREFIX,
    ...
    async def execute(self, action, context) -> AdapterExecutionResult:
        ...
        try:
            ...
            raise SideEffectExecutionError(str(exc.error_code or "b2b_execution_failed")) from exc
        except ToolError as exc:
            raise SideEffectExecutionError(str(getattr(exc, "error_code", "b2b_execution_failed"))) from exc
        external_ref = str((data or {}).get("external_ref") or (data or {}).get("provider_message_id") or "") if isinstance(data, dict) else ""
        return AdapterExecutionResult(
            success=True,
            external_reference=external_ref or f"{self._tool_id}:{action.action_id}",
            reversible=self.reversible,
            rollback_reference=None,
            metadata={"tool_id": self._tool_id, "result": dict(data) if isinstance(data, dict) else {}},
        )

    async def reconcile(self, action, context) -> AdapterReconciliationResult:
        return AdapterReconciliationResult(status=ADAPTER_RECON_UNKNOWN)

    async def rollback(self, action, context) -> RollbackResult:
        return RollbackResult(success=False, rollback_reference=None, metadata={"reason": "not_reversible"})


def register_b2b_commerce_side_effects(
    registry,
    platform_adapter,
    *,
    ...
    for spec in B2B_WRITE_TOOLS:
        adapter = B2BCommerceSideEffectAdapter(
            tool_id=spec["tool_id"],
            operation=spec["operation"],
            capabilities_required=tuple(spec["capabilities"]),
            platform_adapter=platform_adapter,
            external=bool(spec.get("external", True)),
            trust_level=str(trust_level or spec.get("trust_level") or TOOL_TRUST_WRITE_EXTERNAL_REVERSIBLE),
