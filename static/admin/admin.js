...
async function loadView(name) {
  try {
    ...
    if (name === "runs") { setView("runs", "Runs"); return renderTable("/api/admin/ops/runs?limit=50", ["workflow_id", "tenant_id", "status", "workflow_type"]); }
    if (name === "queues") { setView("queues", "Queues"); const q = await api("/api/admin/ops/queues"); document.getElementById("content").innerHTML = `<table><thead><tr><th>lane</th><th>depth</th><th>active</th></tr></thead><tbody>${q.map((r) => `<tr><td>${esc(r.lane)}</td><td>${esc(r.depth)}</td><td>${esc(r.active)}</td></tr>`).join("")}</tbody></table>`; return; }
    if (name === "routing") { setView("routing", "Routing"); const r = await api("/api/admin/ops/routing"); document.getElementById("content").innerHTML = `<p>Status: ${esc(r.status)}</p><p>Active policy: ${esc(r.active_policy_version || "none")}</p>`; return; }
    if (name === "tools") { setView("tools", "Tools"); return renderTable("/api/admin/ops/tools", ["tool_id", "version", "status"]); }
    if (name === "costs") { setView("costs", "Costs"); const c = await api("/api/admin/ops/costs"); document.getElementById("content").innerHTML = `<p>Window: ${esc(c.window)}</p><p>Total cost: ${esc(c.total_cost)}</p>`; return; }
    if (name === "dlq") { setView("dlq", "DLQ"); return renderTable("/api/admin/ops/dlq?limit=50", ["task_id", "tenant_id", "status", "error_code"]); }
    if (name === "approvals") { setView("approvals", "Approvals"); return renderTable("/api/admin/ops/approvals", ["approval_id", "workflow_id", "tenant_id", "status"]); }
    if (name === "tenants") { setView("tenants", "Tenants"); return renderTable("/api/admin/ops/tenants", ["tenant_id", "active_runs", "failed_jobs"]); }
    if (name === "audit") { setView("audit", "Audit"); return renderTable("/api/admin/ops/audit?limit=50", ["timestamp", "actor_ref", "action", "target_id", "result"]); }
    if (name === "alerts") {
